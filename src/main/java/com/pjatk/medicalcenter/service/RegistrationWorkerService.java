...
public class RegistrationWorkerService {
    ...
    public RegistrationWorker getRegistrationWorkerById(long id){
        return registrationWorkerRepository.findById(id).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND,"RegistrationWorker does not exists"));
    }
    ...
    public RegistrationWorker updateRegistrationWorker(RegistrationWorker registrationWorker){
        if(registrationWorkerRepository.findById(registrationWorker.getId()).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "RegistrationWorker does not exists")) != null)
            return registrationWorkerRepository.save(registrationWorker);
    ...
    public void deleteRegistrationWorkerById(long id){
        registrationWorkerRepository.delete(registrationWorkerRepository.findById(id).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND,"RegistrationWorker does not exists")));
    }
}
